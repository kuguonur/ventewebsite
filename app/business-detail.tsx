...
export default function BusinessDetail({ area }: { area: BusinessArea }) {
  ...
  return <main className={`detail-page detail-${area}`}>
    <nav className="detail-nav">
      <a href={`/?lang=${locale}#uzmanlik`} aria-label={tr ? "Vente ana sayfa" : "Vente home"}><img src="/vente-logo.jpg" alt="Vente" /></a>
      <a className="detail-back" href={`/?lang=${locale}#uzmanlik`}>← {tr ? "Faaliyet alanlarına dön" : "Back to business areas"}</a>
      <div className="language-picker" role="group" aria-label={tr ? "Dil seçimi" : "Language selection"}>
        ...
        <button className={!tr ? "active" : ""} onClick={() => setLanguage("en")} type="button">🇬🇧 <span>English</span></button>
      </div>
    </nav>
    ...
    <section className="detail-intro">
      <p className="eyebrow dark"><span /> {tr ? "YAKLAŞIMIMIZ" : "OUR APPROACH"}</p>
      <h2>{copy.intro}</h2>
    </section>

    <section className="detail-services">
      <header><p className="eyebrow dark"><span /> {tr ? "ÇÖZÜMLER" : "SOLUTIONS"}</p><h2>{tr ? "Bu alanda neler yapıyoruz?" : "What do we deliver?"}</h2></header>
    ...
    <section className="detail-process">
      <p className="eyebrow"><span /> {tr ? "ÇALIŞMA MODELİ" : "HOW WE WORK"}</p>
      <h2>{tr ? "Net, bağlantılı ve ölçülebilir bir süreç." : "A clear, connected and measurable process."}</h2>
      <div>{copy.process.map((step, index) => <article key={step}><span>0{index + 1}</span><strong>{step}</strong></article>)}</div>
    </section>

    <section className="detail-cta">
      <p>{tr ? "Bu iş kolunda birlikte çalışmak ister misiniz?" : "Interested in working with us in this area?"}</p>
      <h2>{copy.cta}</h2>
      <a href={`/?lang=${locale}#iletisim`}>{tr ? "İletişime geçin" : "Contact us"} <span>↗</span></a>
    </section>

    <section className="detail-other">
      <p>{tr ? "Diğer faaliyet alanları" : "Other business areas"}</p>
